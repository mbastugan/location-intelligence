"""Export API-shaped JSON for GitHub Pages / static Next.js builds."""
...
from pipeline import DB_PATH, EXPORT_DIR, WEB_PUBLIC_DATA, connect
...
def _latest_metrics(conn, location_id: int):
    rows = conn.execute(
        """
...
def _series(conn, location_id: int, metric_code: str):
    rows = conn.execute(
        """
...
def _scores(conn, location_id: int):
    rows = conn.execute(
        """
...
def build_city_payload(conn, city: dict) -> dict:
    return {
        "slug": city["slug"],
        "name": city["name"],
...
def insight_for(metric_code: str, left: float, right: float, left_name: str, right_name: str) -> str | None:
    rules = {
...
def build_compare(conn, a: dict, b: dict) -> dict:
...
def main() -> None:
    ...
    payloads = {c["slug"]: build_city_payload(conn, c) for c in cities}

    (EXPORT_DIR / "cities.json").write_text(
        json.dumps(index, indent=2, ensure_ascii=False) + "\n", encoding="utf-8"
    )
    for slug, payload in payloads.items():
        (EXPORT_DIR / f"city-{slug}.json").write_text(
            json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8"
        )

    compares = {}
    # Fixed MVP pairs so URLs match the UI (/compare/malaga-vs-alicante).
    pairs = [
        ("malaga", "valencia"),
        ("malaga", "alicante"),
        ("valencia", "alicante"),
    ...
    for left, right in pairs:
        if left not in payloads or right not in payloads:
            continue
        key = f"{left}-vs-{right}"
        compares[key] = build_compare(conn, payloads[left], payloads[right])
        (EXPORT_DIR / f"compare-{key}.json").write_text(
            json.dumps(compares[key], indent=2, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )

    (EXPORT_DIR / "compares.json").write_text(
        json.dumps(list(compares.keys()), indent=2) + "\n", encoding="utf-8"
    )

    for path in EXPORT_DIR.glob("*.json"):
        shutil.copy2(path, WEB_PUBLIC_DATA / path.name)

    conn.close()
    print(f"Exported {len(payloads)} cities and {len(compares)} compares to {EXPORT_DIR}")
    print(f"Copied into {WEB_PUBLIC_DATA}")
